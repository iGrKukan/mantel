#!/bin/bash
# Приёмка Mantel. Запускать НА Mac Studio.
set -uo pipefail
LIB="$HOME/Library/Application Support/Mantel"
CAP="$LIB/Captures"
DESK="$HOME/Desktop"
...
pgrep -f '/Applications/Mantel.app' >/dev/null && ok "процесс Mantel жив" || no "процесс не найден"
...
echo "рабочий документ" > "$DESK/Mantel-обычный-файл.txt"
cp /System/Library/CoreServices/DefaultDesktop.heic "$DESK/отпуск.heic" 2>/dev/null
sleep 4
[ -e "$DESK/Mantel-обычный-файл.txt" ] && ok "txt на месте" || no "txt унесён!"
[ -e "$DESK/отпуск.heic" ] && ok "картинка не-захват на месте" || no "картинка унесена!"
rm -f "$DESK/Mantel-обычный-файл.txt" "$DESK/отпуск.heic"
...
sleep 5
cpu=$(ps -A -o %cpu,comm | grep -i 'Mantel' | grep -v grep | awk '{s+=$1} END {print s+0}')
rss=$(ps -A -o rss,comm | grep -i 'Mantel' | grep -v grep | awk '{s+=$1} END {printf "%.0f", s/1024}')
echo "  CPU=${cpu}%  RSS=${rss} МБ"
awk -v c="$cpu" 'BEGIN{exit !(c<3)}' && ok "CPU в простое < 3%" || no "CPU высокий: $cpu%"

echo "=== окно полки ==="
$GUI osascript -e 'tell application "System Events" to get name of every window of (first process whose bundle identifier is "by.maru.Mantel")' 2>&1 | head -2
