#!/bin/bash
# Архивирует цель ShelfTop-MAS (App Store, песочница) на Mac Studio.
...
ARCHIVE="./build/ShelfTop-MAS.xcarchive"
UID_GUI=$(id -u)
...
sudo -n launchctl asuser "$UID_GUI" sudo -u "$(whoami)" \
    xcodebuild -project ShelfTop.xcodeproj -scheme ShelfTop-MAS -configuration Release \
